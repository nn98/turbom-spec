...
- 인허가 데이터셋(18컬럼)은 정제되어 DB에 적재됨. 이 백엔드는 조회만.
...
└── infra
    ...
    └── sangga/       SangaApiClient, 좌표변환, 응답 매핑 (상권 호출)
```
...
- `LocationSource` 우선순위 재정의(신형 데이터셋 반영): **LICENSE 최상위** — 인허가의 `호실분리여부='Y'`+`호실단위지번주소`가 있으면 관청이 이미 구분한 것이므로 가장 신뢰. 그다음 SANGGA_API(상가 API 층/호 매칭), 최후 OVERLAP_INFERRED(영업기간 겹침 추정).
...
- status: BusinessStatus      // 영업 | 폐업 | 휴업
...
## 6. 인허가 데이터 → 도메인 매핑 (실 CSV 18컬럼, `인허가-데이터-필드명세-v4.md`)

| 도메인 | 소스 컬럼 |
|---|---|
| Site.pnu | PNU (100% 채움, 직접 사용) |
| Site.jibunAddress / roadAddress | 지번주소 / 도로명주소 |
| Site.coordinate | 원본좌표X/Y → WGS84 변환(infra) |
...
5. infra.sangga: SangaApiClient(HTTP), 좌표변환, MarketInfoService(4.2).
