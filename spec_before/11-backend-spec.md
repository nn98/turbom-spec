...
DDL은 `schema-v3.sql` 참조(Site—Unit—Tenancy 3계층 + 보강 필드 + tenancy_market_info/site_radius_store_cache 반영). 이전 `schema.sql`(2계층)·`schema-v2.sql`(site-level neighborhood_snapshot)은 폐기.
...
응답 스키마·에러 코드·예시는 `api-spec-v3.md` 그대로. 재기술 금지, 그 문서를 따른다.
...
- **캐싱 필수**: 자리 좌표 기준 반경조회 원본 리스트를 사이트 단위로 캐시(`site_radius_store_cache` 테이블, schema-v3.sql). 물건마다 다시 반경조회를 부르지 않고, 캐시된 리스트에서 업종만 다르게 필터링해 `tenancy_market_info.same_category_nearby_count`를 채운다.
...
- 3개 엔드포인트가 계약(`api-spec-v3.md`) 예시와 동일 형태로 응답.
...
2. 엔티티·리포지토리·스키마(`schema-v3.sql`, Site-Unit-Tenancy 3계층).
