...
| 상권정보(상가 API, 동 단위 매칭) | 히스토리 보강(층/호/업종) | `units[].industryDetail` 등, `timeline[].industryDetail` 등 |
...
| industryDetail | string\|null | 폐업 이력은 원천적으로 null |
| enrichmentSource | `"sangga_api"`\|`"license_only"` | |
