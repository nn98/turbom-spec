...
businessStatus/licensed_at 신뢰도 신호(`reliabilitySignal`)를 신규 지원. 구현 중 발견해 같이
고친 것 2건: (1) `SiteQueryService.search()`의 기존 후보 필터 버그(units 비어있으면 무조건 제외 —
UNLOCATED 축이 생기며 신흥동 등 실제 매장이 검색에서 누락되던 문제, `!units().isEmpty() ||
!unlocatedRegistrations().isEmpty()`로 수정) — 새 버그가 아니라 원래 있던 버그가 이번 구현으로
노출되어 같이 고친 것. (2) §16 Unit 충돌감지/재분리(`UnitGrouper`)가 `BusinessTypeRegistry`를
몰라 관련 인허가 페어링을 겹침만으로 잘못 재분리하던 문제 수정(등록된 관련쌍은 겹침 검사 제외) —
§16의 "알려진 한계"(창곡동 559-4 더조은병원/아워홈)가 이걸로 해소됨. 기존 98개 테스트 + 신규
테스트 전부 통과. 커밋: `turbom-server` `a13e34d`. 상세: `의사결정-기록.md` §11·§16,
`backend-spec.md` §3.1.
...
단일 substring(`LIKE '%query%'`)으로만 비교하던 방식을 폐기. 신설한 `AddressQuery`
(`domain.site`, JPA 비의존 순수 클래스)가 공백 기준 토큰화 후 모든 토큰이 순서·인접 무관하게
AND 매칭되는지 판정하고, 숫자만인 토큰은 앞뒤 숫자 경계 체크(`(?<!\d)토큰(?!\d)`)를 추가로
건다. DB 사전 필터는 최장 토큰(`anchorToken`)만으로 `LIKE` 쿼리 걸고, 실제 AND 판정은
애플리케이션 레이어(`TenancyQueryService.searchSites`)에서 수행 — 최장 토큰 매칭은 전체 AND
매칭의 필요조건이라 후보가 새지 않음. PNU 정확 일치는 토큰 매칭과 무관하게 항상 통과(PNU는
주소 텍스트에 literal로 안 들어있어서). 기존 방식은 토큰 순서가 바뀌거나 중간 토큰(구/동 등)
하나만 빠져도 매칭이 실패했고, 숫자만 검색하면 지번이 아닌 층/호수 등 무관한 숫자에도 걸렸음.
`api-spec.md` ① 매칭 방식 절 + `backend-spec.md` §4.1/§5 반영. 신규 8+2=10개 테스트 포함 전체
94개 테스트 통과 확인 후 배포. 커밋: `turbom-server` `b0fcfbe`.

### 2026-07-22 (23차) — PNU 산여부 자기모순 83,244건 데이터 교정 완료 (프로덕션 DB)

바로 아래 조사 기록의 후속. 사용자 확인 후 실제 교정 적용: 전체 테이블 백업(524MB) →
83,244건(자기모순 중 산여부 1→0 방향만, 이상값 102건 제외)의 `pnu` 11번째 자리를 `1`→`0`으로
일괄 교정 → 감사 재실행으로 검증(자기모순 83,346→102건, 필지 단위 충돌 8,121→3,214개) →
원 사례(운중동 1034) 재검색으로 단일 사이트 병합 확인. 남은 3,214개 필지 페어는 산/비산 정상
페어(한국 지적 체계상 별개 필지)와 진짜 버그가 섞여있을 수 있어 이번엔 구분 안 함. 상세:
`의사결정-기록.md` §14.

### 2026-07-22 (조사 기록 — 번호 미부여, 코드 변경 없음) — PNU 산여부 자체정합성 감사 결과

14장(의사결정-기록.md)에서 재발이 확인된 산여부 충돌을, 권위 파일 없이 각 행 자신의
`jibun_address`를 현재 파서로 재파싱해 자기모순을 잡는 방식으로 전수 감사(158만 행). 결과:
**83,346건**이 저장된 `pnu`의 산여부 자리와 재파싱 결과가 다름 — 그중 83,244건이 "텍스트에
'산' 없는데 산여부=1로 저장됨"(반대 방향 오류 0건)이라 완전히 일방향. 실제 UPDATE는 아직
안 하고 조사만 완료, 사용자 확인 대기. 상세: `의사결정-기록.md` §14.

### 2026-07-22 (22차) — `GET /api/sites/search`의 candidates에 유닛별 층/호 파싱정보 추가 (`turbom-server`)

프론트(`turbom-client`) 요청 반영: 검색 결과가 PNU(자리) 단위 row라 건물별로 묶은 뒤 층/호로
재분리하려면 자리마다 상세 API(`GET /api/sites/{pnu}`)를 추가 호출해야 했음. `candidates[]`에
`units[]`(축약판 — `unitId`/`parsedFloor`/`parsedUnitNo`/`parseConfidence` 4필드만) 배열을 신설해
추가 호출 없이 검색 결과만으로 층/호 재분리가 가능하게 함. `api-spec.md`/`frontend-spec.md`/
`프론트-연동계약서.md` 세 문서 모두 반영. 기존 79개+신규 1개, 80개 테스트 전부 통과 확인 후 배포.
커밋: `turbom-server` `bf64fad`.

**같이 요청됐다가 보류된 것 — `buildingId`**: 같은 요청에 "동일 건물 판별용 안정 키"로
`buildingId` 필드도 요청됐으나, 이 요청의 배경 문서(`ter-view/docs/superpowers/specs/2026-07-18-site-entity-similarity-design.md`)를
따라가 보니 이미 15차("PNU 권위 파일 조인 정정")에서 "유사 항목 매칭으로 우회할 문제가 아니라
진짜 데이터 중복(산여부 자리만 다른 PNU)"이라고 뒤집혔던 결론과 정면으로 배치됨. 게다가 15차 당시
9만여 건 스캔 기준 1,630쌍이던 산여부 충돌이 지금 158만 건 기준 **8,121쌍**으로 늘어나 있어(권위
파일 조인이 이후 커진 데이터엔 재적용 안 된 것으로 추정), `buildingId`를 새로 얹기 전에 이 데이터
중복부터 원인 치료하기로 사용자와 합의. 진행 상황은 `의사결정-기록.md` §14 참고(작업 중).

### 2026-07-20 (21차) — 전 문서 완전 스윕: 상권API 필터 단위 정정 + marketInfo 실값 2필드 백필 + 프론트 계약서 동기화

20차·하우스키핑에 이어 turbom-spec의 나머지 문서(상권조회-API-명세.md, 인허가-데이터-필드명세-v4.md,
FRONTEND-INTEGRATION-CONTRACT.md, 프론트-연동계약서.md, 프론트-제작-프롬프트.md)까지 전부 실제
코드와 대조하는 완전 스윕. turbom-server의 `docs/superpowers/{specs,plans}/`(10개 파일)도 확인 —
전부 완료된 과거 작업의 날짜 있는 설계기록이라 그 시점 기준 서술이 맞음, 수정 불필요(수정하면
오히려 이력 왜곡).

**발견 1 — 상권API 반경조회는 `indsSclsCd`(소분류)가 아니라 `indsLclsCd`(대분류)로 필터링**:
`상권조회-API-명세.md`는 "indsSclsCd로 서버 필터링"이 더 정확하다고 서술했지만, 실제 Java
구현(`SanggaApiClient.countInRadiusByCategory`)은 `indsLclsCd`를 쓴다 — 인허가 소분류(행안부
체계)와 상가API 소분류(소상공인시장진흥공단 체계)는 서로 다른 분류라 공식 매핑표가 없어서,
`IndustryCategoryMapper`가 인허가 소분류 136개를 상가 대분류 10개로 손수 매핑해뒀기 때문(버그
아니라 실제로 겪은 제약). `backend-spec.md` §4.2의 클래스명 오타(`SangaApiClient`→`SanggaApiClient`,
g 두 개)와 메서드명도 같이 정정. `sangga_client.py`(참고용 Python, 실서비스 미연결)는 옛 가정
그대로라 §6에 실제 구현과의 차이표를 추가.

**발견 2 — `marketInfo.totalStoreCount`/`categoryBreakdown`이 실값인데 문서 어디에도 없었음**:
반경조회 응답에서 같이 나오는 실제 필드(대분류별 개수/비중 포함)인데 `api-spec.md`/`frontend-spec.md`/
`프론트-연동계약서.md` 어디에도 없었음 — 아이러니하게 "2026-07-19부터 구식"이라고 자기 표시해둔
`FRONTEND-INTEGRATION-CONTRACT.md`(영문)만 2026-07-10/11 시점에 이미 정확히 문서화해둔 상태였고,
그 갱신이 한국어 "현행" 문서들로 역반영이 안 됐던 것. 세 문서에 백필하고, `프론트-제작-프롬프트.md`의
"리포트 확장 필드" 섹션도 정정(이 필드들을 프론트 전용 mock으로 새로 만들라고 지시하고 있었는데,
이미 실값으로 있으니 그 지시가 틀렸음 — mock 대상에서 제외하도록 수정, `competitionScore`/
`recentOpenCount`/`districtTags`는 여전히 소스 없어 mock 유지가 맞음).

**발견 3 — `timeline[].status`가 고정 3값(`"영업"|"폐업"|"휴업"`) 유니온으로 잘못 타입됨**: 실제
원본 상태명은 5버킷(영업/정상, 폐업, 휴업, 취소/말소/만료/정지/중지, 제외/삭제/전출)이고 표시 시
"영업"만 정규화, 나머지는 원문 그대로 통과(`backend-spec.md` §3.3). `frontend-spec.md`/
`프론트-연동계약서.md`의 TS 타입을 `string`으로 정정 — 이 역시 `FRONTEND-INTEGRATION-CONTRACT.md`가
2026-07-10/11에 이미 고쳐뒀던 걸 한국어 문서만 놓치고 있었음.

**`FRONTEND-INTEGRATION-CONTRACT.md`는 "구식" 표시를 해제**: 위 발견들 덕분에 이 세션 기준으로
한국어 문서와 다시 동기화됨(빠졌던 `noStorefrontRegistrations`/`currentSubCategory`만 추가) —
상단의 "2026-07-19부터 동결" 배너를 "2026-07-20 갱신"으로 교체.

**인허가-데이터-필드명세-v4.md**: 컬럼 19개→23개(업태명 + 파싱필드 4종), 물건분리 방식(파싱된
층/호가 실제 그룹핑 키로 쓰임 — "안 쪼갠다"던 서술은 낡음), 스케일(유효 5만 건→158만 건대)
세 가지가 낡아서 정정 배너 추가. 나머지 본문(마스킹·좌표변환·category 매핑)은 여전히 유효.

**상태**: 완료. 상세: `의사결정-기록.md` §13.
